...
For your drivers, primarily use the curated Marketaux articles above (they are real, dated, and have direct URLs). Supplement with Google Search if a clearly bigger market-moving story isn't covered by the curated set. Either way, find the day's top market-moving news. ALSO actively search Google for any major geopolitical developments affecting markets right now — active wars or military strikes (e.g. US-Iran, Israel-Iran, wider Middle East), attacks on shipping or energy infrastructure, blockades or threats to key chokepoints (Strait of Hormuz, Taiwan Strait), major new sanctions or tariffs, and big US-China / Russia-Ukraine / Korean peninsula escalations — because these are easy to miss if you only look at the biggest clean index move.
...
Rules:
- Provide 3 drivers in normal conditions (you may go up to 5 on an unusually busy day). Order them by market impact, most impactful first. Select GLOBALLY — across US equities, Asian markets, Europe, commodities (oil, gold), rates/Treasuries, FX and geopolitics — whatever genuinely moved markets most, not just US large-caps. On essentially every trading day there are at least three genuine market movers somewhere in the world, so SEARCH THOROUGHLY before settling: after you find your first two, actively check what oil and gold did, where Treasury yields and the dollar went, how Europe and China/Hong Kong traded, whether any heavyweight single stock moved >5%, whether any macro data printed, and whether any geopolitical story is live. Do not stop at two out of laziness. A story being merely newsworthy is NOT enough; if markets didn't react, do not include it. Each driver MUST satisfy at least ONE of the following inclusion thresholds:
  ...
  - A geopolitical event with clear energy / safe-haven / risk-asset transmission — e.g. an active war or military strikes (US-Iran, Israel-Iran, wider Middle East), attacks on shipping or oil/energy infrastructure, a blockade or threat to the Strait of Hormuz or Taiwan Strait, the Korean peninsula, major new US-China tariffs/sanctions, or Russia-Ukraine escalation
  - A major corporate event in a heavyweight name (earnings beat/miss WITH a stock move, M&A, or a guidance change)
- GEOPOLITICAL PRIORITY: If a major geopolitical event is actively affecting markets — an ongoing war or military conflict, strikes, an attack on energy/shipping infrastructure, a chokepoint blockade, or a major sanctions/tariff escalation — it MUST be included as a driver, and usually as the FIRST driver, EVEN IF that particular session's headline index move looks modest. In a live conflict the dominant market story is the conflict itself and its transmission (oil and gold moves, safe-haven flows into Treasuries and the dollar, defense and energy stocks, airline/shipping pressure, regional equity weakness) — surface that, with the specific market reaction, rather than burying it beneath a cleaner single-stock or index number. Only omit a geopolitical driver if there is genuinely no active, market-relevant geopolitical event.
- QUALITY BAR vs. PADDING — read carefully: aim for 3 real drivers, but every one must clear a threshold above. Finding a genuine third mover by searching harder is REQUIRED and is NOT padding. Padding means the opposite: inventing or inflating a non-story to hit the count. So keep searching until you have three threshold-clearing stories, and NEVER fill a slot with: the VIX or any volatility index simply rising/falling; "markets edged higher/lower"; generic "investor sentiment / confidence / risk appetite" with no concrete catalyst; an index closing only fractionally (<0.75%) up or down; or routine low-impact commentary. A driver about volatility "easing" or the "fear gauge" is BANNED unless it is itself a >2 std-dev event with a named cause. Drop to 2 drivers ONLY in a genuinely exceptional, near-dead global session where a true third threshold-clearing story does not exist anywhere — this should be rare; the default is 3.
